...
export async function createUser(db: D1Database, data: { email: string; name: string; password_hash: string }): Promise<RushUser> {
  const row = await db.prepare('INSERT INTO users (email, name, password_hash) VALUES (?, ?, ?) RETURNING *').bind(data.email, data.name, data.password_hash).first<RushUser>();
  if (!row) throw new Error('createUser: INSERT RETURNING returned no row');
  return row;
}
...
export async function searchFiles(db: D1Database, ownerId: number, query: string, limit?: number): Promise<RushFile[]> {
  return db.prepare('SELECT f.* FROM files f JOIN files_fts fts ON f.id = fts.rowid WHERE fts.name MATCH ? AND f.owner_id = ? LIMIT ?').bind(query, ownerId, limit ?? 20).all<RushFile>().then(res => res.results);
}
...
export async function createFile(db: D1Database, data: { name: string; folder_id?: number; owner_id: number; r2_key: string; size: number; mime_type: string }): Promise<RushFile> {
  const row = await db.prepare('INSERT INTO files (name, folder_id, owner_id, r2_key, size, mime_type) VALUES (?, ?, ?, ?, ?, ?) RETURNING *').bind(data.name, data.folder_id, data.owner_id, data.r2_key, data.size, data.mime_type).first<RushFile>();
  if (!row) throw new Error('createFile: INSERT RETURNING returned no row');
  return row;
}
...
export async function createFolder(db: D1Database, data: { name: string; parent_id?: number; owner_id: number; path: string; depth: number }): Promise<Folder> {
  const row = await db.prepare('INSERT INTO folders (name, parent_id, owner_id, path, depth) VALUES (?, ?, ?, ?, ?) RETURNING *').bind(data.name, data.parent_id, data.owner_id, data.path, data.depth).first<Folder>();
  if (!row) throw new Error('createFolder: INSERT RETURNING returned no row');
  return row;
}
...
export async function createUploadSession(db: D1Database, data: { file_name: string; file_key: string; upload_id: string; mime_type?: string; total_parts: number; total_bytes: number; folder_id?: number; owner_id: number }): Promise<UploadSession> {
  const row = await db.prepare('INSERT INTO upload_sessions (file_name, file_key, upload_id, mime_type, total_parts, total_bytes, folder_id, owner_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING *').bind(data.file_name, data.file_key, data.upload_id, data.mime_type ?? '', data.total_parts, data.total_bytes, data.folder_id, data.owner_id).first<UploadSession>();
  if (!row) throw new Error('createUploadSession: INSERT RETURNING returned no row');
  return row;
}
